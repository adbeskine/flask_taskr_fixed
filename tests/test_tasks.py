...
import unittest
from Flask_taskr import app, db, bcrypt
...
class TasksTests(unittest.TestCase):
    ...
    def register(self, name, email, password, confirm):
        return self.app.post(
            '/register',
            data=dict(username=name, email=email, password=password, confirm=confirm),
    ...
    def create_user(self, name, email, password):
        new_user = User(username=name, email=email, password=bcrypt.generate_password_hash(password))
        db.session.add(new_user)
        db.session.commit()

    def create_task(self):
        return self.app.post('/add_task/', data=dict(
            name='Go to the bank',
            due_date='02/05/15',
            priority='1',
            posted_date='02/04/15',
            status='1'
        ), follow_redirects=True)
    ...
    def test_users_can_see_task_modify_links_for_tasks_created_by_them(self):
        ...
        self.app.get('/tasks/', follow_redirects=True)
        response=self.create_task()
        self.assertIn(b'mark_complete/1/', response.data)
    ...
    def test_admin_users_can_see_task_modify_links_for_all_tasks(self):
        ...
        response = self.create_task()
        self.assertIn(b'mark_complete/1/', response.data)
        self.assertIn(b'mark_complete/2/', response.data)
        self.assertIn(b'delete_task/1/', response.data)
        self.assertIn(b'delete_task/2/',response.data)
        user_task = db.session.query(Task).filter_by(task_id=1).first()
        self.assertEqual(user_task.poster.username, 'Michael')
        admin_task = db.session.query(Task).filter_by(task_id=2).first()
        self.assertEqual(admin_task.poster.username, 'Superman')
